...
d3.queue()
    .defer(d3.json, "data/ny-quantize-topo.json")
    .defer(d3.json, "data/income.json", function(d) {
        // for(var i=0;i<d.length;i++) {
        //     console.log(typeof d[i].id);
        // }
        for(var i =0;i<d.length;i++) {
            if (isNaN(d[i].income)) {
                incomeData.set(d[i].id.toString(), 0);
            } else {
                incomeData.set(d[i].id.toString(), +d[i].income);
                console.log(incomeData);
            }
        }
    })
    .defer(d3.csv, "data/poverty.json", function(d) {
        for(var i =0;i<d.length;i++) {
            if (d[i].poverty == '-') {
                povertyData.set(d[i].id, 0);
            } else {
                povertyData.set(d[i].id, +d[i].poverty);
            }
        }
    })
    .await(ready);
